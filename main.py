...
def main(inputFolder, outputFolder, categoryNbr):
    device = 'cuda' #'cpu' #torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    version_num = 0
    base_dir = "/localdata/htc22"
    path_parts = [base_dir, "lpd_unet_sub", "range=" + str(30), "version_" + str(version_num)]
    ...
    model.load_state_dict(torch.load(os.path.join(log_dir, "best_model.pt"),  map_location=torch.device('cpu')))
    model.eval()
    model.to(device)
